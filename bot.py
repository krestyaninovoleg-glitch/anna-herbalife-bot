...
TOKEN: Final[str] = os.getenv("BOT_TOKEN", "8539857651:AAEZONa579W_JtU_tR2BV4CRS6AI52P4ueQ")
ADMIN_CHAT_ID: Final[str] = os.getenv("ADMIN_CHAT_ID", "500574875")
SITE_LINK: Final[str] = "https://a-yakubenko.com"
WHATSAPP_LINK: Final[str] = "https://wa.me/97450146509?text=Здравствуйте!Хочу получить консультацию по Herbalife."
TELEGRAM_BOT_LINK: Final[str] = "https://t.me/anna_yakubenko_bot"
CONTACT_PHONE_TEXT: Final[str] = "+974 5014 6509"
...
PRODUCT_BLOCKS = {
    "weight": (
        "🎯 <b>Для снижения веса</b>\n\n"
        "• Formula 1\n"
        "• Active Fiber Complex\n"
        "• Protein Drink Mix\n"
        "• Formula 1 Express Bar\n\n"
        "Подходит для тех, кто хочет более удобный и контролируемый рацион."
    ),
    "energy": (
        "⚡ <b>Для энергии</b>\n\n"
        "• Formula 1\n"
        "• Напитки Herbalife\n"
        "• Грин Макс Select\n\n"
        "Подходит, если хочется чувствовать больше тонуса и бодрости."
    ),
    "routine": (
        "🥗 <b>Для комфортного рациона</b>\n\n"
        "• Formula 1\n"
        "• Protein Bake Mix\n"
        "• Protein Drink Mix\n"
        "• Протеиновые чипсы\n\n"
        "Хороший вариант для повседневного удобного питания."
    ),
    "muscle": (
        "💪 <b>Для белка и восстановления</b>\n\n"
        "• Protein Drink Mix\n"
        "• Спортивное питание Herbalife24\n"
        "• Протеиновые перекусы\n\n"
        "Подходит при тренировках и активном образе жизни."
    ),
}

CATALOG_TEXT = (
    "📦 <b>Каталог Herbalife</b>\n\n"
...
PROMO_TEXT = (
    "🎁 <b>Спецпредложение</b>\n\n"
    "Оставьте заявку, и Анна поможет:\n"
    "• подобрать продукты под цель\n"
    "• объяснить удобный маршрут старта\n"
    "• выбрать удобный формат связи"
)
...
FAQ_TEXT = (
    "❓ <b>Частые вопросы</b>\n\n"
    "• Как быстро можно получить консультацию?\n"
    "Обычно можно начать общение сразу через WhatsApp или оставить заявку.\n\n"
    "• Можно ли подобрать программу под цель?\n"
    "Да, бот подскажет направление, а Анна уточнит детали лично.\n\n"
    "• Можно ли написать в Telegram по номеру телефона?\n"
    "Нет, прямой переход в Telegram по номеру телефона не поддерживается как надёжная публичная ссылка. "
    "Для Telegram лучше использовать username или ссылку на бота."
)
...
def main_inline_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("🤖 Открыть бота", url=TELEGRAM_BOT_LINK)],
...
def contact_inline() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("📲 Написать в WhatsApp", url=WHATSAPP_LINK)],
            [InlineKeyboardButton("🤖 Открыть Telegram-бот", url=TELEGRAM_BOT_LINK)],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = (
        ...
        "• подобрать направление под цель\n"
        "• оставить заявку\n"
        "• быстро перейти в WhatsApp или на сайт\n\n"
        "Выберите, что вас интересует."
    )
    await update.message.reply_text(text, reply_markup=MAIN_MENU)
    await update.message.reply_text("Быстрый старт:", reply_markup=main_inline_menu())
...
async def products(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = (
        ...
        "• <b>Active Fiber Complex</b>\n"
        "• <b>Protein Bake Mix</b>\n"
        "• <b>Formula 1 Express Bar</b>\n"
        "• <b>Protein Chips</b>\n"
        "• <b>Herbalife24</b>\n\n"
        "Выберите цель, и я подскажу удобное направление."
    )
...
async def contact(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = (
        f"📞 <b>Контактный номер:</b> {CONTACT_PHONE_TEXT}\n"
        f"📲 <b>WhatsApp:</b> {CONTACT_PHONE_TEXT}\n"
        f"🤖 <b>Telegram:</b> используйте ссылку на бота\n"
        f"🌐 <b>Сайт:</b> {SITE_LINK}\n\n"
        "Важно: Telegram не даёт надёжно открыть чат по номеру телефона как обычную внешнюю ссылку. "
        "Поэтому для Telegram используется username и ссылка на бота."
    )
    await update.effective_message.reply_text(
        text,
        parse_mode=ParseMode.HTML,
...
async def lead_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.effective_message.reply_text(
        ...
        "Можно отправить текстом или нажать кнопку ниже.",
...
async def lead_note(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.effective_message.reply_text(
        "Пока ждёте ответ, можете сразу перейти в WhatsApp или снова открыть бота:",
        reply_markup=contact_inline(),
    )
    context.user_data.clear()
...
async def handle_goal_pick(update: Update, context: ContextTypes.DEFAULT_TYPE, goal_key: str) -> None:
    ...
    block = PRODUCT_BLOCKS.get(goal_key, "Анна поможет подобрать подходящий вариант.")
    text = (
